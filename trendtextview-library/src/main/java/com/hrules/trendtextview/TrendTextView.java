...
public class TrendTextView extends TrendFontTextView {
    ...
    private SpannableString getSpannableStringWithCursor(SpannableString text) {
        ...
        spannableString.setSpan(span, spannableString.length() - 1, spannableString.length(), Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
    ...
    private void applyLetterSpacing() {
        ...
        if (stringBuilder.toString().length() > 1) {
            for (int i = 1; i < stringBuilder.toString().length(); i += 2) {
                spannableString.setSpan(new ScaleXSpan((letterSpacing + 1) / 10), i, i + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            }
        }
